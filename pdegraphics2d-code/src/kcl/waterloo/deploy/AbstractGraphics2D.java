 ...
 *
 * AbstractGraphics2D is extended e.g. by PDEGraphics2D which provides a
 * concrete implementation for generating Processing script output.
 *
...
public abstract class AbstractGraphics2D extends Graphics2D {
    ...
    /**
     * Graphics2D instance supplied to an AbstractGraphics2D subclass
     * constructor.
     */
     ...
     */
    protected AbstractGraphics2D(Graphics2D g) {
        parentGraphics = (Graphics2D) g.create();
    ...
    public String getCode() {
        String str = pictures.get(0).toString();
        if (str.length() > 65535) {
            System.out.println("Warning: this script is too long for use in Processing.\n");
        }
        return str;
    }
     ...
     * if (g instanceof AbstractGraphics2D) {<br>
     * ((AbstractGraphics2D)g).setPaintObject(this);<br>
     * }<br>
     * }<br></p>
     *
     * @param paintObject the paintObject to set
     */
    public abstract void setPaintObject(Object referenceObject);
    ...
    /**
     * Inserts user-specified code at the start of the generated code, but after any
     * automatically generated declarations (which are therefore potentially
     * available for use in that code).
     *
     * @param comment fully formatted code string
     */
    ...
    public void copyArea(int x, int y, int width, int height, int dx, int dy) {
        System.err.println("copyArea" + width + " x " + height);
    ...
    private void markAsDirty(int x, int y, int width, int height, ImageObserver observer) {
        if (observer != null) {
            if (observer instanceof JComponent) {
                RepaintManager mgr = RepaintManager.currentManager((JComponent) observer);
                mgr.addDirtyRegion((JComponent) observer, x, y, width, height);
            } else if (observer instanceof Window) {
                RepaintManager mgr = RepaintManager.currentManager((Window) observer);
                mgr.addDirtyRegion((Window) observer, x, y, width, height);
            } else if (observer instanceof JApplet) {
                RepaintManager mgr = RepaintManager.currentManager((JApplet) observer);
                mgr.addDirtyRegion((JApplet) observer, x, y, width, height);
            } else {
            }
        }
    }
    ...
    public boolean drawImage(Image img, int x, int y, int width, int height, ImageObserver observer) {
        boolean result = parentGraphics.drawImage(img, x, y, width, height, null);
        markAsDirty(x, y, width, height, observer);
        return result;
    }
    ...
    public boolean drawImage(Image img, int x, int y, int width, int height, Color bgcolor, ImageObserver observer) {
        boolean result = parentGraphics.drawImage(img, x, y, width, height, bgcolor, null);
        markAsDirty(x, y, width, height, observer);
        return result;
    }
    ...
    public boolean drawImage(Image img, int dx1, int dy1, int dx2, int dy2, int sx1, int sy1, int sx2, int sy2, Color bgcolor, ImageObserver observer) {
        boolean result = parentGraphics.drawImage(img, dx1, dy1, dx2, dy2, sx1, sy1, sx2, sy2, bgcolor, null);
        markAsDirty(dx1, dy1, dx2 - dx1, dy2 - dy1, observer);
        return result;
    }

    @Override
    public boolean drawImage(Image img, AffineTransform xform, ImageObserver obs) {
        boolean result = parentGraphics.drawImage(img, xform, null);
        markAsDirty((int) xform.getTranslateX(), (int) xform.getTranslateY(), img.getWidth(obs), img.getHeight(obs), obs);
        return result;
    }
