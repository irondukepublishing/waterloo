 ...
 * <h3>Scrollable</h3>
 * <p>
 * {@code JXPanel} is {@link Scrollable} by default. It provides reasonable
 * implementations of all of the interface methods. In addition, it supports the
 * setting of common scrolling approaches defined in {@link ScrollableSizeHint}.
 * </p>
 * <h3>Alpha Support</h3>
 * <p>
 * {@code JXPanel} has full alpha-channel support. This means that the
 * {@code JXPanel} can be made fully or partially transparent. This means that
 * the JXPanel and all of its children will behave as a single paint at the
 * specified alpha value. <strong>Cauton:</strong> best practice is to use
...
//    private boolean scrollableTracksViewportWidth = true;
     ...
     * If no painter is specified, the normal painting routine for JPanel is
     * called. Old behavior is also honored for the time being if no
    ...
    /**
     * Creates a new <code>JXPanel</code> with a double buffer and a flow
     * layout.
     */
    public JXPanel() {
    }
    ...
    /**
     * Creates a new <code>JXPanel</code> with <code>FlowLayout</code> and the
     * specified buffering strategy. If <code>isDoubleBuffered</code> is true,
     * the <code>JXPanel</code> will use a double buffer.
     *
     ...
     * {@code 1f} (full opaqueness). Setting the component to be non-opaque will
     * restore the previous alpha transparency. If the component is non-opaque
     * with a fully-opaque alpha value ({@code 1f}), the behavior should be the
     * same as as a {@code JPanel} that is non-opaque.
     */
    @Override
    ...
    /**
     * Sets the horizontal sizing hint. The hint is used by the Scrollable
     * implementation to service the getScrollableTracksWidth.
     *
     * @param hint the horizontal sizing hint, must not be null and must be
     * vertical.
     *
    ...
    /**
     * Sets the vertical sizing hint. The hint is used by the Scrollable
     * implementation to service the getScrollableTracksHeight.
     *
     * @param hint the vertical sizing hint, must not be null and must be
     * vertical.
     *
     * @throws NullPointerException if null
     *
     ...
     * NONE, if the boolean parameter is true or false, respectively.<p>
     *
     * <b>NOTE</b>: this method is kept for backward compatibility only, for
     * full control use setScrollableHeightHint.
     *
     * @param scrollableTracksViewportHeight The scrollableTracksViewportHeight
     * to set.
     *
     * @see #setScrollableHeightHint(ScrollableSizeHint)
     */
     ...
     *
     * <b>NOTE</b>: this method is kept for backward compatibility only, for
     * full control use setScrollableWidthHint.
     *
     *
     * @param scrollableTracksViewportWidth The scrollableTracksViewportWidth to
     * set.
     *
     * @see #setScrollableWidthHint(ScrollableSizeHint)
     */
