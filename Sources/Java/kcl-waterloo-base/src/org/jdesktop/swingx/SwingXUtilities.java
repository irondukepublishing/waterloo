 ...
 *
 * <li> move this class to the swingx utils package which already has a bunch of
 * xxUtils
 * <li> move methods between xxUtils classes as appropriate (one window/comp
 * related util)
...
public final class SwingXUtilities {
     ...
     * @param pressed the name of the action in the action map to invoke when
     * the mnemonic is pressed
     ...
     * @param pressed the name of the action in the action map to invoke when
     * the mnemonic is pressed
     * @param released the name of the action in the action map to invoke when
     * the mnemonic is released (if the action is a toggle style, then this
     * parameter should be {@code null})
     * @throws NullPointerException if the component is {@code null}
     */
     ...
     * @param enabled {@code true} if the component is to enabled; {@code false}
     * otherwise
     */
...
//        }
    ...
    /**
     * A version of {@link SwingUtilities#invokeLater(Runnable)} that supports
     * return values.
     *
    ...
    /**
     * A version of {@link SwingUtilities#invokeAndWait(Runnable)} that supports
     * return values.
     *
     * @param <T> the return type of the callable
     * @param callable the callable to execute
     ...
     * event dispatching thread to finish executing {@code callable.call()}
     ...
     * ancestor exists. This method also returns {@code null} if any parameter
     * is {@code null}.
     */
    @SuppressWarnings("unchecked")
     ...
     * hierarchy. If a parent in the chain is of type JPopupMenu, the parent
     * chain of its invoker is walked.
     *
     * @param focusOwner
     * @param parent
     * @return true if the component is contained under the parent's hierarchy,
     * coping with JPopupMenus.
     */
     ...
     * @return true if the given property should be replaced by the UI's default
