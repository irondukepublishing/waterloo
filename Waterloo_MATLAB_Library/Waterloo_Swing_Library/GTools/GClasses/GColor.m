classdef GColor
    ...
    methods(Static)
        ...
        function color=getWeightedAverage(color1, w1, color2, w2)
            w1=abs(w1);
            w2=abs(w2);
            if ismatlab(color1)
                color=((color1*w1) + (color2*w2))/2;
            else
                red=(color1.getRed()*w1 + color2.getRed()*w2) /(w1+w2);
                green=(color1.getGreen()*w1 + color2.getGreen()*w2) /(w1+w2);
                blue=(color1.getBlue()*w1 + color2.getBlue()*w2) /(w1+w2);
                color=java.awt.Color(int16(min(red,255)),int16(min(green,255)),int16(min(blue,255)));
            end
        end
        ...
        function color=getMonochrome(color, n, llim)
            ...
            % by varying the lightness of the reference color
            % Example:
            % color=getMonochrome(color, n)
            %   where color on input is the starting color and
            %   n=number of colors to return
            %
            % Terminate the series at higher lightness by specifying llim on
            % input:
            % color=getMonochrome(color, n, llim)
            %       where llim/2=proportion of the reference color lightness
            %       to use (default=1).
            [color, mlflag]=GColor.toJava(color);
            if nargin<3
                llim=1;
        ...
        function color=toRGB(hsl)
            ...
            if S==0
                ...
                var1=2*L-var2;
                R=int16(Hue2RGB(var1,var2,H+1/3)*255);
                G=int16(Hue2RGB(var1,var2,H)*255);
                B=int16(Hue2RGB(var1,var2,H-1/3)*255);
