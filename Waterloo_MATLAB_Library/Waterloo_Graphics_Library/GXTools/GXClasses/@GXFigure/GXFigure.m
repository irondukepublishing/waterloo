classdef GXFigure < hgsetget
    ...
    % Changes:
    % 27.11.2012 loading kclf files fixed for LinkedHashMap output [1.1Alpha3]
    ...
    methods
        ...
        function newObj=save(obj, fileName, showProgress)
            ...
            hgjava=findall(obj.Parent', 'Type', 'hgjavacomponent');
            if (~isempty(hgjava))
                set(hgjava, 'UserData',[]);
            end
        ...
        function newObj=saveDialog(obj, str)
            ...
            % Use specified folder if available
            if nargin==2
                wd=str;
            else
                wd='';
            end
            % Else use last folder...
            fc=kcl.waterloo.gui.file.FileChooser.getInstance();
            if (isempty(wd))
                wd=fc.getCurrentDirectory().getPath();
            end
            %... or default to pwd
            if (isempty(wd))
                wd=pwd();
            end
            val=kcl.waterloo.gui.file.FileChooser.createSaveDialog(wd);
            switch val
    ...
    methods (Static)
        ...
        function newObj=openDialog()
            ...
            val=kcl.waterloo.gui.file.FileChooser.createOpenDialog();
            switch val
                case javax.swing.JFileChooser.APPROVE_OPTION
                    fc=kcl.waterloo.gui.file.FileChooser.getInstance();
                    selection=char(fc.getSelectedFile().toString());
                    ...
                    if strcmpi(ext, '.kclfig')
                        ...
                        try
                            % Fixed for LinkedHaskMap output [1.1Alpha3]
                            newObj=kcl.waterloo.xml.GJDecoder.load(selection);
                            frame=newObj.get('Graphics').createFrame();
                            frame.setPreferredSize(newObj.get('Dimension'));
                            frame.setSize(newObj.get('Dimension'));
                        catch
                        end
                    end
            end
