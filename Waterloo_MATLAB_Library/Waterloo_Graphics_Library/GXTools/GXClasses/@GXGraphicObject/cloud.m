function plotobj=cloud(target, X, Y, S, C, varargin)
% scatter method for GXGraphicObject objects
% Examples:
% cloud(GXGraphicObject, X,Y,S,C) 
% cloud(GXGraphicObject, X,Y)
% cloud(GXGraphicObject, X,Y,S)
% cloud(...,markertype) 
% cloud(...,'filled') 
% cloud(...,'PropertyName',propertyvalue)
% 
% See also: scatter
%
% ---------------------------------------------------------------------
...
if ~isempty(varargin)
    ...
    elseif strcmpi(varargin{1}, 'linespec')==0 && isLineSpec(varargin{1})
        varargin=horzcat('LineSpec', varargin);
    end
    try
    TF=cellfun(@strcmpi, varargin, repmat({'UsePolar'}, size(varargin)));
    if any(TF)
        isPolar=true;
        idx=find(TF);
        varargin(idx)=[];
...
if isPolar
    plotobj=GXPlot(target, 'polarcloud', props);
else
    plotobj=GXPlot(target, 'cloud', props);
end

if isFilled
    plotobj.getObject().setFill(plotobj.getObject().getFill());
end
