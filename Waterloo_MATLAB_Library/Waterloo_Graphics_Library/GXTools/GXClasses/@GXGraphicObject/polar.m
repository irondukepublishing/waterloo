function plotobj=polar(target, X, Y, varargin)
...
if props.containsKey('Marker') || props.containsKey('MarkerFcn') || props.containsKey('LineSpec')
    plotobj=GXPlot(target, 'polarline', props);
    sc=kcl.waterloo.plot.WPlot.polar(props);
    % Link data in 1.1Alpha
    sc.getPlot().setXData(plotobj.getObject().getXData());
    sc.getPlot().setYData(plotobj.getObject().getYData());
