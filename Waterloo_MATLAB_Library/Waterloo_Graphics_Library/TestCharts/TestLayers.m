function [f, ax1, ax2, ax3]=TestLayers()
...
f=GXFrame('GXFrame with multi-layered plot',1,1);
f.getObject().setLocation(f.getObject().getX()+150,f.getObject().getY()+150);
ax1=f.getObject().getContentPane().add(GJGraphContainer.createInstance(javaObjectEDT(GJGraph.createInstance())));
...
ax1.get().setAxisBox(true);


ax2=ax1.get().getView().add(javaObjectEDT(kcl.waterloo.graphics.GJGraph.createInstance()));
t= 0:.035:2*pi;
